...
for seed in range(seed_begin, seed_end):
    ...
    command = f'mpirun -np 1 --use-hwthread-cpus Rscript compute_hessinv.r {max_children} sim-output/npy_files_{seed:04d}_{max_children:02d} {seed_results_path} {" ".join(map(str, optim))} &> {seed_results_path}/hesslog.out'
